...
module.exports = function gameMode (req, res) {
  request('https://en.wikipedia.org/w/api.php?action=query&format=json&list=random&rnnamespace=0&rnlimit=5', function(error,response,data) {
    if (!error && response.statusCode == 200) {
      ...
      var wikiReqUrl = 'https://en.wikipedia.org/w/api.php?action=query&prop=extracts&format=json&exintro=&explaintext=&exsentences=3&pageids=';

      request(wikiReqUrl + firstTitleId, function(error,response, firstTitleData) {
        request(wikiReqUrl + secondTitleId, function(error,response, secondTitleData) {
          request(wikiReqUrl + thirdTitleId, function(error,response, thirdTitleData) {
            request(wikiReqUrl + fourthTitleId, function(error,response, fourthTitleData) {
